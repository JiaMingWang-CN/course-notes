...
> **Course-notes bundled adaptation (modified Apache-2.0 copy):** This workflow and its sibling domain skills are already in `references/hyperframes-skills/`. Do not run `hyperframes skills update`, look in home-installed skill directories, or fetch workflow instructions from elsewhere. The repository's `SKILL.md` and `references/COURSE_VIDEO.md` govern shared production and HTML/MP4 output. Use `HYPERFRAMES_SKIP_SKILLS=1` with CLI `init`; other CLI dependencies still require consent to install.
...
`HYPERFRAMES_SKIP_SKILLS=1 npx hyperframes init "videos/<project>" --non-interactive --example=blank --skill=faceless-explainer` — use the equivalent environment-variable syntax on the active shell; this avoids touching global Agent skills. When `course-notes` has already designated a per-section `hyperframes/` project path, use that path instead of `videos/<project>`.
