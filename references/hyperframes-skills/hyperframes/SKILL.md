...
> **Course-notes bundled adaptation (Apache-2.0 copy, modified):** When invoked through the repository's `SKILL.md`, the ten domain skills and the `general-video` / `faceless-explainer` workflows are siblings under `references/hyperframes-skills/`. Resolve `/hyperframes-*`, `/media-use`, `/general-video` and `/faceless-explainer` to their sibling `SKILL.md` files here; resolve relative `references/` and `scripts/` under those directories. The repository's `SKILL.md` section 6 and `references/COURSE_VIDEO.md` own intake, output choice, and user approval. Do not install/update Agent skills, load from the user's home directory, or route to an unbundled workflow; for other video intents ask before changing scope. For `init`, set `HYPERFRAMES_SKIP_SKILLS=1`. CLI/library dependencies are still external software, not external skill instructions.
...
## 4. Enter the bundled workflow (course-notes adaptation)

For course-note teaching videos read the bundled `../faceless-explainer/SKILL.md` (short invented-visual lesson, approximately up to three minutes) or `../general-video/SKILL.md` (longer lesson or footage edit), then its referenced local files. **Never run `hyperframes skills update` or load an installed Agent skill from outside this bundle.** The repository's `SKILL.md` section 6 selects output (interactive HTML / MP4 / both), and `references/COURSE_VIDEO.md` adapts the review/render steps. If the source workflow requires an unbundled specialty, ask before changing scope rather than silently loading it.
